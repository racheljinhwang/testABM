...
/**
 * 
 * This class allows to read GBFS and convert it to the input file needed by the
 * sharing contrib. Currently this is only applicable to the station-based
 * services.
 *
 */
...

		{
			...
			if (stationsNode.isArray()) {
				for (JsonNode stationNode : stationsNode) {
					...
					int capacity = 100;//stationNode.findValue("capacity").asInt();
					...
					double distance = CoordUtils.calcEuclideanDistance(coord, linkCoord);
					if (distance > 200)
						continue;
...

		{
			...
			if (stationsNode.isArray()) {
				for (JsonNode stationNode : stationsNode) {
					...
					// we need to check if we have this station or not

					if (!stationLinks.containsKey(Id.create(stationId, SharingStation.class)))
						continue;
